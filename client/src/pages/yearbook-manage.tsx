...
import { ArrowLeft, Upload, Plus, Trash2, Settings, Eye, BookOpen, FileText, Layers, Send as Publish, ChevronUp, ChevronDown, Edit, Menu, ShoppingCart, LogOut } from "lucide-react";
...
export default function YearbookManage() {
  ...
  // Force reload on first visit to yearbook manage page
  useEffect(() => {
    if (schoolId && year) {
      const firstVisitKey = `yearbook-manage-first-visit-${schoolId}-${year}`;
      const hasVisited = sessionStorage.getItem(firstVisitKey);
      
      if (!hasVisited) {
        // Mark as visited to prevent infinite reload
