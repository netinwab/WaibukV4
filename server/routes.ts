...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/yearbooks/:yearbookId/upload-page", upload.single('file'), async (req, res) => {
    try {
      ...
      const imageUrl = `/api/secure-image/yearbooks/${req.file.filename}`; // Use secure endpoint
  ...
  app.post("/api/payments/initialize", async (req, res) => {
    try {
      ...
      // Validate required fields
      if (!email || !amount || !cartItems || !userId) {
        return res.status(400).json({ 
          status: false, 
          message: "Missing required fields: email, amount, cartItems, userId" 
        });
      }
      ...
      const paystackData = {
        ...
        reference,
        currency: 'NGN',
        callback_url: `https://${process.env.REPLIT_DEV_DOMAIN || 'localhost:5000'}/api/payments/verify/${reference}`,
        metadata: {
